    ...
    try:
        ...
        cursor.execute(
            ...
            INSERT OR REPLACE INTO profiles (
                email, fullname, phone, age, address, bio, company, position, website, gender, photo
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''',
            (
                session['email'],
...
def update_profile():
    ...
    try:
        ...
        photo_file = request.files.get('photo')
        new_photo = save_uploaded_photo(photo_file)
        ...
        profile_data = _load_profile_by_email(session['email'])
        return render_template('profile.html', profile=profile_data, error='Unable to update profile.')
